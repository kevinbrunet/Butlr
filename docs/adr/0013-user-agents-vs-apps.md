...
Elle **ne marche pas telle quelle** pour Carson, l'UI web, l'app mobile, ou un interrupteur physique configuré niveau 2. Ces composants ne sont **pas** des sources d'intention autonome — ils **transmettent l'intention de l'utilisateur**. Quand Kevin dit « Hey Carson, allume la lumière », l'auteur de l'intention c'est **Kevin**, pas Carson.

Cette confusion a une conséquence directe :

- Si Carson est traité comme une app, il faudrait lui accorder une permission par device, et il accumulerait des VDevices niveau 1. Ce n'est ni le sens fonctionnel ni la sémantique souhaitée par Kevin (cf. session de conception 2026-04-25).
- L'ADR 0007 pose que **le niveau 2 est réservé aux intentions de l'utilisateur** ; l'ADR 0009 pose qu'**une app ne peut pas s'auto-déclarer niveau 2**. Ces deux règles se contredisent si Carson est une app **et** émet des niveau 2.
...
- **Carson** (assistant vocal). L'utilisateur parle, Carson identifie le `actor_user_id` (au POC : utilisateur unique du foyer ; Phase 3 : speaker diarization, cf. `architecture.md §11`).
...
```
{
  ...
  "via_agent_id":  "carson" | "ui-web" | "ui-mobile" | "switch:salon",
...
- **Carson** : si l'utilisateur dit « allume la lumière » sans préciser la durée :
  - Soit Carson **calcule** une durée selon une heuristique configurée (par device et par contexte temporel : « lumière du salon le soir » = jusqu'à minuit ; « radiateur en hiver » = 1 h).
  - Soit Carson **demande explicitement** à l'utilisateur (« Pour combien de temps ? »).
  - **Jamais** Carson n'émet un niveau 2 sans durée — il est rejeté par l'orchestrateur.
...
Heuristique de calcul Carson (hors scope de cet ADR — détaillée dans la spec de Carson) : possible de pondérer par contexte de présence, plage horaire, type de device.

### Authentification des agents-utilisateur

L'ADR 0009 reportait l'authentification cryptographique d'**app**. Le sujet est **plus critique** pour les agents-utilisateur, car eux peuvent émettre du niveau 2 — il faut s'assurer qu'un process malveillant ne peut pas se faire passer pour Carson et émettre des niveau 2 arbitraires.
...
- **Audit lisible.** Une question "pourquoi le thermostat est à 22 ?" trouve une réponse en deux temps : "à 18:33 Kevin a posé un override 22°C pour 2h — via Carson en vocal".
- **Pas de pollution de permissions.** Carson ne demande pas la permission de piloter chacun des 50 devices du foyer un par un. Il **est** l'utilisateur.
- **Symétrie naturelle entre canaux.** Carson, UI web, UI mobile, interrupteur niveau 2 → même rôle, même payload, même trace.
...
- **Charge UX pour Carson.** L'heuristique de calcul de durée et le pattern "demander quand on ne sait pas" sont à concevoir et tester avec soin — c'est là que se joue la qualité perçue de l'assistant vocal.
- **Cas limite : automation déclenchée par voix.** Si un jour Carson permet à l'utilisateur de **créer des automations** vocalement (« quand je rentre, mets la lumière à 50% »), il faudra distinguer le cas "Carson exécute pour Kevin" (niveau 2 ponctuel) du cas "Carson installe une app au nom de Kevin" (création d'une app autonome niveau 1 dans le système). Hors scope POC, mais à garder en tête.

## Alternatives considérées

### A. Carson est une app interne avec permission auto-octroyée niveau 2 sur tous les devices

Tentation initiale (mon premier jet, cf. tasks-vdevice-implementation.md §2.7.1 avant correction). Rejeté : viole l'ADR 0009 ("une app ne peut pas s'auto-déclarer niveau 2") et brouille la sémantique. Le niveau 2 cesse d'être "ce que l'utilisateur a fait" pour devenir "ce que Carson a fait au nom de l'utilisateur, peut-être" — perte de lisibilité.

### B. Carson en niveau 1 avec très haute priorité

Plus simple, contourne le problème de niveau. Rejeté : un override vocal n'est pas une intention persistante, c'est un acte ponctuel. Le mettre niveau 1 le ferait persister jusqu'à ce qu'une autre app le préempte — exactement ce que Kevin ne veut pas (« si je dis carson allume la lumière, cela va passer comme un override temporaire »).
...
### D. Modèle "OAuth scope" : Carson tient un token utilisateur signé qui prouve son authority
...
Plus simple. Rejeté : on perd la traçabilité du canal, qui est précisément l'information utile pour comprendre des comportements anormaux (« mon thermostat a été overridé — par Carson ou par mon UI ? »).

## Révisions

- **2026-04-25** — Création. Issue d'une correction de Kevin lors de la session du 2026-04-25 : "carson n'est pas une app comme les autres, il agit au nom d'un utilisateur". Patche en aval les ADRs 0008 et 0009 (notes de révision ajoutées dans ces fichiers) ainsi que `vdevice-architecture.md` et `tasks-vdevice-implementation.md`.
- **2026-04-25** — Patch terminologique suite à [ADR 0014](0014-dynamic-tiers-arbiters.md). La typologie agents-utilisateur vs apps autonomes **reste structurellement valide** — c'est une typologie de citoyens, indépendante du nombre de niveaux. Mais sa **règle d'application** s'exprime désormais par **tags d'admission** plutôt que par numéro de niveau hardcodé : un VDevice posé par un agent-utilisateur porte le tag `user_agent` ; un VDevice posé par une app autonome porte le tag `app`. Les niveaux du système sont configurés (preset par défaut + overrides) avec des `admission.tags_required` qui décident qui peut y poser un VDevice. La règle "niveau 2 réservé aux agents-utilisateur" devient "le niveau `user-override` du preset par défaut admet `user_agent` uniquement". Le payload reste : `actor_kind`, `actor_user_id`, `via_agent_id`, `app_id` ; **un champ `tags` peut être ajouté** ou dérivé automatiquement de `actor_kind` au point d'entrée (à arbitrer dans la spec API). Le payload garde aussi un champ `tier_id` optionnel (cf. ADR 0014) — si non fourni, l'orchestrateur résout le niveau de plus haut rang dont l'admission matche les tags.

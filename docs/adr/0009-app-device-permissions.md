...
- **2026-04-25** — Patch suite à ADR 0013 (agents-utilisateur vs apps). Le **modèle de permission décrit ici concerne les apps autonomes uniquement**. Les **agents-utilisateur** (Carlson, UI web/mobile, interrupteurs configurés niveau 2) **ne sont pas soumis au prompt de permission par device** — ils agissent au nom de l'utilisateur lui-même, donc l'utilisateur ne s'auto-demande pas l'autorisation. La règle "une app ne peut pas s'auto-déclarer niveau 2" reste vraie et est **renforcée** par l'ADR 0013 : seuls les agents-utilisateur peuvent émettre niveau 2. L'authentification cryptographique des **agents-utilisateur** est un sujet **plus critique** que pour les apps tierces (car ils émettent du niveau 2) — voir ADR 0013 §"Authentification des agents-utilisateur" : ADR séparé à ouvrir Phase 2+.
- **2026-04-25** — Patch terminologique suite à [ADR 0014](0014-dynamic-tiers-arbiters.md). La règle "une app ne peut pas s'auto-déclarer niveau 2" devient "la création d'un VDevice échoue si la `admission.tags_required` du niveau ciblé n'est pas satisfaite par les tags du VDevice". Le tag `app` ne matche pas un niveau qui exige le tag `user_agent` (par exemple le niveau `user-override` du preset par défaut). La sémantique est strictement préservée, exprimée désormais par config plutôt que par hardcoding. La **permission par couple `(app_id, device_id)`** s'enrichit d'un champ `tier_max` (identifiant nommé) au lieu de `level_max` numérique. Le stockage des permissions migre de SQLite vers des fichiers yaml en git ([ADR 0015](0015-config-git-yaml-hierarchy.md) §"Apps et permissions") — le modèle fonctionnel reste identique, seul le format change. Persistance des permissions désormais sous `~/.butlr/config/permissions/<app_id>__<device_id>.yaml`.

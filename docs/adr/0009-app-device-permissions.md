...
- **2026-04-25** — Patch suite à ADR 0013 (agents-utilisateur vs apps). Le **modèle de permission décrit ici concerne les apps autonomes uniquement**. Les **agents-utilisateur** (Carson, UI web/mobile, interrupteurs configurés niveau 2) **ne sont pas soumis au prompt de permission par device** — ils agissent au nom de l'utilisateur lui-même, donc l'utilisateur ne s'auto-demande pas l'autorisation. La règle "une app ne peut pas s'auto-déclarer niveau 2" reste vraie et est **renforcée** par l'ADR 0013 : seuls les agents-utilisateur peuvent émettre niveau 2. L'authentification cryptographique des **agents-utilisateur** est un sujet **plus critique** que pour les apps tierces (car ils émettent du niveau 2) — voir ADR 0013 §"Authentification des agents-utilisateur" : ADR séparé à ouvrir Phase 2+.

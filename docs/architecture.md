...
```
Butlr/
├── carson/          # Python — agent vocal Pipecat : wake word → STT → LLM → TTS, client MCP
└── mcp-home/         # .NET 10 — serveur MCP : outils de pilotage de la maison
```

Carson est client MCP. mcp-home est serveur MCP **long-running**, lancé indépendamment (service systemd / Windows Service / `dotnet run` en dev). Au démarrage, Carson ouvre une connexion **SSE/HTTP** vers mcp-home (`MCP_HOME_URL`, défaut `http://localhost:5090/mcp` ~), s'authentifie par bearer token (`MCP_HOME_TOKEN`), liste les tools exposés et les traduit en function schemas OpenAI-compatible pour le LLM local. Un tool_call émis par le LLM est proxifié vers mcp-home via ce même canal HTTP (cf. ADR 0003).

La topologie cible est **Carson satellite ↔ mcp-home central**. On assume cette topologie dès le POC pour éviter une migration transport ultérieure, même si les deux processus tournent souvent sur la même machine au démarrage.

### Pourquoi deux projets séparés (et pas un seul)
- **Découplage matériel** : Carson est GPU-bound (LLM + Whisper), mcp-home est lightweight. On peut héberger mcp-home sur un mini-PC / Raspberry Pi central sans GPU et bouger Carson sur la machine GPU, sans couplage.
- **Responsabilité unique** : mcp-home est un service stateless (ou presque) qui répond à des appels MCP ; Carson est un pipeline audio temps réel. Contraintes d'ops et cadences de release différentes.
...
- Sortie : un fichier `.tflite` dropable dans `carson/assets/wakeword/`.

**Option B — Utiliser un wake word existant**
- Moins bon pour l'identité "Carson" mais zéro setup. À écarter si tu tiens au nom.
...
Le system prompt de Carson contient :
...
1. Disant « Hey Carson, allume le salon » → Carson répond, la console de mcp-home logge `turn_on_light room=salon`.
...
- **Tool discovery automatique** : Carson liste les tools au démarrage, le LLM reçoit un function schema à jour sans redéploiement couplé.
...
- **Piste multi-agents** (cf. §11) : si Carson devient orchestrateur de plusieurs agents spécialisés, chaque agent est un serveur MCP parmi d'autres — la mécanique de tool discovery scale naturellement.
...
| Piper en MVP TTS | Latence, packaging trivial | Voix moins expressive — Carson aura une voix "fonctionnelle" pas "majordome" avant l'upgrade |
...
- **Commande manuelle d'urgence** : les devices doivent rester contrôlables sans Carson (interrupteur physique, app HA). Non négociable.
...
- **Orchestrateur multi-agents** : Carson devient un chef d'orchestre qui route les demandes vers plusieurs agents spécialisés (agent domotique, agent cuisine, agent agenda, agent recherche…). Chaque agent = un serveur MCP distinct. `mcp-home` n'est alors qu'un parmi d'autres — la mécanique de tool discovery scale naturellement. Côté LLM, deux options à arbitrer le moment venu : (a) exposer au LLM principal tous les tools agrégés, (b) mettre en place un router/planner qui choisit d'abord l'agent, puis délègue. (b) scale mieux mais ajoute de la latence.
- **Mémoire longue durée** : Carson a besoin de se souvenir de préférences ("mets toujours 21°C quand je dis froid"). Solution probable : un petit store clé-valeur, exposé comme tool MCP (`remember`, `recall`). Phase 2+.
...
- **Interruption** : si l'utilisateur coupe Carson au milieu d'une phrase, Pipecat gère l'interruption au niveau frame ✓ mais le comportement doit être testé sur les cas de tool calls en cours (annulation propre).
...
3. **Slice Carson 1** — pipeline Python : STT → LLM (llama.cpp server Qwen 7B) → TTS. Push-to-talk en dev, pas encore de wake word. Vérifier que "quelle heure est-il ?" marche bout en bout, sans tool.
4. **Slice Carson 2** — brancher le client MCP de Carson sur mcp-home, exposer les tools au LLM. Dire "allume le salon" → l'instruction s'affiche dans la console de mcp-home. Premier vrai tool calling end-to-end.
5. **Slice Carson 3** — wake word custom "Hey Carson" entraîné avec openWakeWord. Remplace le push-to-talk.
6. **Slice Carson 4** — sidecar filler + pré-narration. Tester la perception sur des tool calls artificiellement ralentis (ex. `Task.Delay(1500)` côté mcp-home).

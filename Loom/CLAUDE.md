...
| TTS | Pocket TTS (bibliothèque, `TTSModel`, `loom_orchestrator/tts_pocket.py`) | Modèle `french_24l` ou `french` (~12 couches par défaut, pas 6 — corrigé après vérification doc officielle, cf. Révisions ADR-0036) selon benchmark T2.1. ⚠ Pas encore de clonage par locuteur : `PocketTtsSynthesizer` utilise une unique voix FR de repli (`estelle`) en attendant T3.1-T3.3 ; voix clonées prévues en `.safetensors` (`export_model_state`/`import_model_state`). |
...
- `loom_orchestrator/tts_pocket.py` — `PocketTtsSynthesizer`, même statut que `translation_seamless.py` : composant du pipeline, pas un outil de mesure. ⚠ API vérifiée par documentation officielle seulement (pas de lecture de code source ni d'exécution locale — Pocket TTS n'est installé/testable que sur la machine cible), voix de repli unique (`estelle`), pas de clonage par locuteur.

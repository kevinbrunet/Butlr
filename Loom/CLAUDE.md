...
**Stade actuel : T1.4 validé, politique de commit AlignAtt implémentée (ADR-0041), pas encore exécutée sur la machine cible.** STT WLK confirmé fidèle sur corpus réel (2026-07-14). Traduction NLLB abandonnée (hallucinations récurrentes) au profit de SeamlessM4T v2, validé sur la machine cible (2026-07-15, repli boucles de répétition corrigé). Diarisation Sortformer/NeMo installée et validée sur un run à 2 locuteurs avec recouvrement (2026-07-15) : `speaker_id` stables, contamination de contenu résiduelle en zone de recouvrement (cf. ADR-0034). Premier harnais WLK→Seamless→Pocket TTS écrit (`bench/harness_pipeline.py`) : une première politique de commit (attendre la fin de la ligne WLK) s'est révélée structurellement fausse pour un locuteur unique continu (cf. ADR-0041) ; une revue de littérature (2026-07-15) a montré que AlignAtt (attention croisée du décodeur, sans ré-entraînement) est déjà validé sur SeamlessM4T (SimulSeamless, FBK) et même déjà utilisé en interne par le STT de WLK — `harness_pipeline.py` réécrit pour committer un increment de traduction dès qu'AlignAtt le juge sûr (`loom_orchestrator/alignatt.py`, `translation_seamless.AlignAttSeamlessTranslator`), pas encore testé en conditions réelles (forme exacte des tensors d'attention `transformers` non vérifiée par exécution, cf. Révisions ADR-0041). Sonde TTS isolée (`bench/harness_tts.py`) : time-to-first-chunk dans le budget de 400ms sur des phrases courtes/moyennes, mais un warning `Maximum generation length reached without EOS` et un nombre de chunks très variable sur la même phrase suggèrent un problème de fin de génération à investiguer avant de faire confiance au composant. ADR-0042 (2026-07-15, pas encore implémentée) : séparation de voix (SepFormer-WHAMR) + suivi d'identité par embedding, pour la contamination de contenu en zone de recouvrement déjà documentée (ADR-0034) — vient s'insérer entre l'extraction audio d'une ligne WLK et son envoi à Seamless, sans toucher à la diarisation WLK elle-même. Voir le backlog complet transmis par Kevin (Phases 0-4) pour le détail des tâches.
...
- `docs/adr/` — décisions d'architecture spécifiques à Loom, numérotées dans la séquence globale du dépôt (cf. `../.claude/rules/adr-writing.md`) :
  ...
  - `0041` — Politique de commit AlignAtt (attention Seamless), découplée des lignes WLK — **ADR de référence pour la segmentation avant traduction**
  - `0042` — Séparation de voix (SepFormer) + suivi d'identité par embedding, pour la contamination en zone de recouvrement — **ADR de référence pour la robustesse au chevauchement, pas encore implémentée**

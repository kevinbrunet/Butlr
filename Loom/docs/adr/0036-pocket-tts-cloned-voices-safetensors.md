...
- 2026-07-15 — correction suite à lecture de la doc officielle (kyutai-labs.github.io/pocket-tts, github.com/kyutai-labs/pocket-tts) pour câbler `loom_orchestrator/tts_pocket.py` : ~ le modèle `french` par défaut est annoncé 12 couches (pas 6 comme supposé initialement en Context ci-dessus), `french_24l` reste la variante plus grande/plus lente — corrige l'hypothèse "~" non vérifiée, mais reste basé sur la doc, pas une exécution réelle (cf. révision suivante). Un premier composant (`tts_pocket.PocketTtsSynthesizer`) utilise une voix de repli unique (`estelle`, un des noms courts documentés) plutôt qu'un clonage par locuteur — T3.1-T3.3 (export de voix par locuteur) toujours pas commencés, ce composant n'est qu'un point de départ pour valider le câblage bout-en-bout (`bench/harness_pipeline.py`), pas une implémentation de la décision de cet ADR sur le clonage.
- 2026-07-15 — correction par exécution réelle (premier run de `harness_pipeline.py` sur la machine cible) : la révision précédente était fausse sur ce point précis — `TTSModel.load_model(language="french")` échoue (`ValueError: For technical reasons, only a larger 24-layer model is available for French`). ✓ Il n'existe pas de variante FR plus légère que 24 couches ; `tts_pocket.py` utilise désormais `"french_24l"` sans alternative. Rappel méthodologique : la doc officielle listait `french`/`french_24l` comme deux options symétriques sans préciser cette exception par langue — un run réel a été nécessaire pour la découvrir, la doc seule ne suffisait pas ici.
- 2026-07-25 — correction par exécution réelle sur la machine cible : `synthesize_continuation`/`copy_state=False` (chaînage des increments d'une même ligne pour la continuité prosodique, introduit avec le découplage chunking Seamless/TTS du 2026-07-15) fait dégénérer Pocket TTS en boucle audio — un mot/une courte phrase répété des dizaines de secondes. Confirmé par STT (`faster-whisper`) et à l'oreille, à la fois sur le run de production (`corpus a`, `main.py`) et sur une sonde isolée (`bench/harness_tts_continuation.py`) : reproduit 5/5 en isolation, y compris juste après un reset de l'état vocal — écarte les hypothèses de longueur de continuation et de contention GPU testées avant (`--reset-every`, `--gpu-contention`). ✓ Aucun usage de `copy_state=False` trouvé ailleurs dans l'écosystème Pocket TTS (ni les mainteneurs — issue kyutai-labs/pocket-tts#151, chunks générés indépendamment —, ni zeropointnine/tts-audiobook-tool, toujours `copy_state=True`) : chemin de code manifestement peu exercé en pratique, contrairement au chemin par défaut. `commit_state.py`/`main.py`/`bench/harness_pipeline.py`/`bench/harness_pipeline_dual.py` basculés vers `synthesize_stream` (`copy_state=True`, le défaut) — chaque increment repart de l'état de voix de base, plus de continuation. Conséquence acceptée : rupture de prosodie/débit à chaque frontière d'increment au sein d'une ligne — pas de mitigation en place (crossfade au mixage, increments plus longs via `commit_policy.py` envisagés, pas encore mesurés). `new_line_state`/`synthesize_continuation` conservés dans `tts_pocket.py` uniquement comme sonde de régression (`bench/harness_tts_continuation.py`).
